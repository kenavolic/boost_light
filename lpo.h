...
template <> inline float lexical_cast<float>(const std::string &str) {
  return std::stof(str);
}

template <> inline int lexical_cast<int>(const std::string &str) {
  return std::stoi(str);
}

template <> inline long lexical_cast<long>(const std::string &str) {
  return std::stol(str);
}

template <> inline long long lexical_cast<long long>(const std::string &str) {
  return std::stoll(str);
}

template <>
inline unsigned int lexical_cast<unsigned int>(const std::string &str) {
...
template <typename... Ts> class program_options {
private:
  ...
  template <typename T, typename = void> struct pos_opt {
    using value_type = std::decay_t<T>;
    std::string desc;
    T *val;
  };
...
public:
  ...
  explicit program_options(const std::string &prog, const std::string &desc)
      : m_prog{prog}, m_global_desc{desc} {}

  ///
  /// @brief Register a new flag (i.e. options set or not without value like
  /// --version -v)
  ///
  program_options &add_flag(flag_opt_t &&flag);
  ...
  ///
  template <typename T> program_options &add_pos_opt(pos_opt<T> &&pos_opt);
...
private:
  template <typename T>
  bool has_arg_match(const T &opt, const std::string &arg) const {
    return (("--" + opt.name) == arg) ||
           (!opt.short_name.empty() && (("-" + opt.short_name) == arg));
  }

  template <typename Opt> bool in_range(const Opt &o) const {
  ...
  void add_opt(const std::string &name, const std::string &short_name) {
    if (has_opt(name, short_name)) {
      throw std::runtime_error(
          "duplicate option name or short name for option " + name);
    }
...
template <typename T>
inline auto operator<<(std::ostream &os, const T &obj)
    -> decltype(std::declval<T>().print(os), os) {
  obj.print(os);
  return os;
}
...
program_options<Ts...> &program_options<Ts...>::add_flag(flag_opt_t &&flag) {
...
template <typename... Ts>
bool program_options<Ts...>::parse(int argc, char **argv) {
  ...
  for (args_count = 1; args_count < argc; ++args_count) {
    ...
    match = std::any_of(std::cbegin(m_flag_list), std::cend(m_flag_list),
                        [&arg, this](auto &p) {
    ...
    for (auto &v : m_opt_list) {
      try {
            ...
            [&arg, &match, &args_count, &args, &argc, this](auto &&opt) {
              ...
              if (has_arg_match(opt, arg)) {
                ...
                if constexpr (std::is_arithmetic_v<T>) {
                  if (!in_range(opt)) {
                    throw std::runtime_error("out of range value for option <" +
                                             opt.name + ">");
                  }
                }
        ...
        std::cerr << "[-] value option parsing failure: " << ex.what()
                  << std::endl;
        return false;
      } catch (...) {
  ...
  for (unsigned i = 0; i < m_pos_list.size(); ++args_count, ++i) {
    ...
    try {
      ...
      std::cerr << "[-] positionnal arg parsing failure: " << ex.what()
                << std::endl;
      return false;
    } catch (...) {
